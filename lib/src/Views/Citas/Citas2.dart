...
class Citas2 extends StatefulWidget {
  const Citas2({Key? key}) : super(key: key);

  @override
  _Citas2State createState() => _Citas2State();
}

class _Citas2State extends State<Citas2> {
    ...
    if (response['status'] == 'success') {
      ...
      if (tipoCita == 'remota' && eventoId != null) {
        ...
        try {
          ...
          print('Error al eliminar el evento en Google Calendar: $e');
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error al eliminar el evento en Google Calendar.'),
            ),
          );
        }
      }
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            ...
                            String? eventoId = cita['id_evento']; // Puede ser null
